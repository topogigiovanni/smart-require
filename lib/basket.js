/*global document, XMLHttpRequest, localStorage, smartRequire, RSVP*/
(function(window, document) {
	'use strict';

	var head = document.head || document.getElementsByTagName('head')[0];
	var storagePrefix = 'smartRequire-';
		...
		try {
			...
			if (e.name.toUpperCase().indexOf('QUOTA') >= 0) {
				...
				if (tempScripts.length) {
					...
					smartRequire.remove(tempScripts[0].key);
	...
	var isCacheValid = function(source, obj) {
		return !source ||
			...
			(smartRequire.isValidItem && !smartRequire.isValidItem(source, obj));
	};
	...
	var handleStackObject = function(obj) {
		...
		source = smartRequire.get(obj.key);
		...
		if (obj.live || shouldFetch) {
			if (obj.unique) {
				...
				obj.url += ((obj.url.indexOf('?') > 0) ? '&' : '?') + 'smartRequire-unique=' + obj.unique;
			}
			promise = saveUrl(obj);
	...
	window.smartRequire = {
		...
		removeHandler: function(types) {
			smartRequire.addHandler(types, undefined);
		}
	};
